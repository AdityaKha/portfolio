...
@Component({
  selector: 'app-resume',
  standalone: true,
  imports: [CommonModule, FadeInDirective, PdfViewerWrapperComponent],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <section id="resume" class="section-padding">
      <div class="w-full max-w-6xl mx-auto px-6">

        <!-- Heading + buttons pinned at vertical center; preview expands below without moving them -->
        <div class="flex flex-col items-center justify-center min-h-[calc(100vh-8rem)]">
          ...
          <!-- Action buttons -->
          <div class="flex flex-wrap items-center justify-center gap-3" appFadeIn>
            <a
              [href]="resumePath"
            ...
            <button
              (click)="togglePreview()"
              class="btn-outline text-sm py-3 px-6 flex items-center gap-2 min-w-[12.5rem] justify-center"
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        @if (previewOpen()) {
          <div #previewContainer class="glass rounded-2xl border border-white/10 overflow-hidden shadow-2xl preview-fade-in">
            ...
            @defer (when previewOpen()) {
              <app-pdf-viewer-wrapper [src]="resumePath" />
            } @placeholder {
              <div class="flex items-center justify-center py-16">
                <span class="text-text-secondary text-sm">Loading preview…</span>
              </div>
            }
          </div>
        }
...
export class ResumeComponent {
  ...
  togglePreview() {
    ...
    if (opening) {
      setTimeout(() => {
        this.previewContainer?.nativeElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }, 50);
    }
  }
}
